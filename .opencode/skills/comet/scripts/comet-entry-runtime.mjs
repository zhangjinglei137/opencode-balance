...
// domains/comet-entry/workflow-resolution.ts
var COMET_WORKFLOW_RESOLUTION_SCHEMA = "comet.workflow-resolution.v1";
async function resolveCometWorkflowResolution(startPath) {
  return {
    schema: COMET_WORKFLOW_RESOLUTION_SCHEMA,
    ...await resolveCometEntry(startPath)
  };
}
function formatCometWorkflowResolution(resolution) {
  return [
...
// domains/comet-entry/entry-runtime.ts
var USAGE = "Usage: comet-entry-runtime [path] [--json]";
function parseEntryRuntimeArgs(args) {
  let help = false;
  let json = false;
  let targetPath = null;
  ...
  return { help, json, targetPath: path4.resolve(targetPath ?? ".") };
}
async function runCometEntryRuntime(args, io = {
...
`);
  ...
  try {
    ...
    io.stdout(
      parsed.json ? `${JSON.stringify(resolution, null, 2)}
...
// domains/comet-entry/entry-runtime-entry.ts
